...
camera.position.set(0, 0, 0); // 摄像机位于中心
// 注意：当相机位于中心时，lookAt(0,0,0) 无效，初始朝向由 controls 或默认决定
const renderer = new THREE.WebGLRenderer({ antialias: true, alpha: false });
renderer.setSize(window.innerWidth, window.innerHeight);
renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2)); // 限制过高 pixel ratio 以防性能问题
...
async function discoverImages() {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error('无法读取 PicturesList.txt');
        }
        const text = await response.text();
        const lines = text.split('\n').filter(line => line.trim() !== '');
        
        console.log(`找到 ${lines.length} 个图片路径`);
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            try {
                ...
                console.log(`✅ 加载图片：${url} (${img.width}x${img.height})`);
                const info = { index: i + 1, url, img, width: img.width, height: img.height };
                results.push(info);
                addImageToWall(info);
                ...
                if (infoEl) {
                    ...
                    infoEl.textContent = `加载中：${i + 1}/${lines.length} (${pct}%) - 已加载 ${results.length} 张图片`;
                }
        ...
        console.log(`共加载 ${results.length} 张图片`, results.map(v => v.url));
        return results;
    } catch (err) {
        console.error('读取图片列表失败:', err);
        NProgress.done();
        if (infoEl) {
            infoEl.textContent = '加载失败，请检查 PicturesList.txt 文件';
        }
        return [];
    }
}
...
function addImageToWall(info) {
    ...
    // 使用 Texture 而不是 CanvasTexture 以兼容 Image 对象
...
discoverImages().then(images => {
    if (images.length === 0) {
        console.error('未找到任何图片，请确认 /image/ 目录下存在 PicturesList.txt 中列出的文件');
    }
}).catch(err => {
